...
public class GradientDecline {
    ...
    private static final double[][] y = {{1, 1, 1}, {1, 0, 1}, {1, 0, 4}, {1, 1, 0},
    ...
    public static void main(String[] args) {
        ...
        for(int k = 0; k < 2; ++k) {
            ...
            double[] curA = minus(lastA, times(perceptronFunction(lastA, y, classes), thetha(k)));
            as.add(curA);
        }
        for(double[] vec : as) {
    ...
    public static double[] times(double[] x, double times) {
    ...
    public static double[] perceptronFunction(double[] a, double[][] ys, int[] actualClassification) {
        ...
        for(int i = 0; i < ret.length; ++i) {
            for(Integer wrongClassifiedIdx : wrongClassifications) {
                ret[i] -= g(a, y[wrongClassifiedIdx]);
            }
        }
        return ret;
    }
